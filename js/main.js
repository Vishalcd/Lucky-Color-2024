...
const html = function (data) {
  const html = `
          <div class="result__details">
            <h4 class="result__details-period">${(data.gameId - 1).toString().padStart(2, "0")}</h4>
            <h4 class="result__details-price result__details-price--${data.gameResult}">
              ${data.gameResult === "win" ? "+" : "-"}₹${data.gameBetAmount}
            </h4>
            <h4 class="result__details-number">${data.gameBetNumber
              .toString()
              .padStart(2, "0")}</h4>
            <h4 class="result__details-result result__details-result--${data.gameBetColor}">
              <span></span>
            </h4>
          </div>`;
...
function warning() {
  ...
  setTimeout(() => {
    ...
    document.querySelector(".bet__container").classList.remove("animation-active");
  }, 2000);
}
/////////////////////////////////////////////////////////////////////
...
amountTypeButton.forEach((button) => {
  button.addEventListener("click", (e) => {
    ...
    document.querySelector(".bet__controls-increase").dataset.curramount = `+${amountType}`;

    //bet decrease function
    document.querySelector(".bet__controls-decrease").dataset.curramount = `-${amountType}`;
  });
});
...
amountButtons.forEach((button, i) => {
  button.addEventListener("click", (e) => {
    //guard clause
    if (button.classList.contains("bet__controls-decrease") && +inputAmount.value < 10) return;
...
confirmButton.addEventListener("click", function (e) {
  ...
  if (+inputAmount.value > +myMoney.textContent) {
    document.querySelector(".not-enough-money").textContent = "Not enough balance";
    warning();
    return;
  }
  ...
  window.setTimeout(() => {
    ...
    ///////////////////////////
    if (
      (+inputAmount.value <= +myMoney.textContent && +inputAmount.value >= 10 && userBet.color) ||
      userBet.number
    ) {
      /////////////////////////
      ...
      if (userBet.color && userBet.color === randomColors) {
        myMoney.textContent = `${Number(myMoney.textContent) + Number(userBet.betAmount)}`;
      ...
      /////////////////////////
        ...
        myMoney.textContent = `${Number(myMoney.textContent) + Number(userBet.betAmount) * 8}`;
      ...
      //loss cheak function
      /////////////////////////
      else if (userBet.color !== randomColors && userBet.number !== randomNumber) {
        myMoney.textContent = `${Number(myMoney.textContent) - Number(userBet.betAmount)}`;
